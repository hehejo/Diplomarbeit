...
Der nächste Abschnitt prüft das eingeführte Verarbeitungsmodell anhand einiger Verteilungsstrategien auf seine Tauglichkeit. Eine Evaluation möglicher Algorithmen zur Implementierung andere Policies sprengt den Rahmen dieser Arbeit. Zudem haben die anderen Policies keinen Einfluss auf die Verteilung eines Events, sondern lediglich der Nachrichtenverarbeitung auf einem Knoten dienen. Die Verteilung der einzelnen Nachrichten hat allgemein einen höheren Stellenwert, da sie eine Auswirkung auf die Anzahl der zu verschickenden Nachrichten im System hat.

\subsection*{Beispielhafte Strategien}
Für jede einzelne Policy bietet \ac{m2etis} mehrere konkrete Implementierungen an und lässt sich mit benutzerdefinierten Strategien weiter auf die Anforderungen der Applikation anpassen\footnote{siehe \Fref{fig:metis_aufbau}}. Die näher betrachtete Verteilungsalgorithmen orientieren sich an Scribe, das in \Fref{chap:related:scribe} vorgestellt wurde, und an VON, das auch bereits in \Fref[plain]{chap:related:von} beschrieben ist. Scribe erzeugt einen Multicast-Tree, während VON seinen Fokus auf die \enquote{in-game}-Nachbarschaft legt. Dies ist vor allem bei Events mit einer hohen Lokalität sinnvoll. Der Einfluss solcher Events, wie zum Beispiel Positionsänderungen, wirkt sich nur in einem begrenzten Gebiet aus und muss daher nur benachbarten Knoten -- im Spiel -- mitgeteilt werden.
