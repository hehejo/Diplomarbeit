\subsection[Umsetzung eines kanalbasierten Systemes]{Umsetzung eines kanalbasierten Systemes am Beispiel von Scribe}
\label{chap:related:scribe}
Scribe \cite{Castro2002Scribe} selbst setzt auf dem strukturierten Overlay-Netzwerk Pastry \cite{Rowstron2001} auf und erzeugt einen vom Subscriber zum Publischer aufgebauten Baum \emph{reverse path forwarding tree} \cite{Dalal1978}. Dieses Struktur ist ein Multicast-Tree\index{Multicast-Tree} genannt, das ist ein durch die Knoten im Netz gebildeter Baum über den Nachrichten verteilt werden. Pro Kanal erzeugt Scribe einen eigenen Multicast-Tree.
